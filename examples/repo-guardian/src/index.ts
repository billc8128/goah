...
const runner = new ProcessRunner(model
  ? { command: process.execPath, args: [piWorkerPath()], env: { GOAH_PI_MODEL: model, GOAH_PI_PROVIDER: process.env.GOAH_PI_PROVIDER ?? "anthropic", ...(process.env.ANTHROPIC_API_KEY ? { ANTHROPIC_API_KEY: process.env.ANTHROPIC_API_KEY } : {}), ...(process.env.OPENAI_API_KEY ? { OPENAI_API_KEY: process.env.OPENAI_API_KEY } : {}), GOAH_PI_ALLOW_BASH: "true" } }
...
const supervisor = new Supervisor(ledger, runner, new class { now(): Date { return new Date(); } }(), { workspace: new GitWorkspaceManager(repo), heartbeatPolicies: [{ agent: "guardian", maxSilentMs: 172_800_000, escalateTo: "human" }] });
...
if (process.argv.includes("--daemon")) {
  ...
  await supervisor.runAvailable(1);
